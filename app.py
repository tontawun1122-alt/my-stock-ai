...
def ll(l):
    ...
    return m.get(l,("—","—"))[0 if LANG=="TH" else 1]
...
def run_ai(api_key, ticker, d, lang):
    ...
    # ── CRITICAL: Force language in system instruction AND prompt ──────────────
    lang_rule = (
        "⚠️ ABSOLUTE RULE: You MUST write EVERY text field ENTIRELY in Thai language (ภาษาไทย). "
        "ALL analysis, summary, risks, catalysts MUST be Thai. ZERO English sentences allowed in text fields. "
        "Company names and financial numbers can stay in English, but ALL explanatory text = Thai only."
...
with st.sidebar:
    ...
    with cl:
        if st.button("🌐 TH/EN"):
            st.session_state.lang="EN" if LANG=="TH" else "TH"; st.rerun()
    with ct:
...
if btn and tickers_in:
    ...
    for tk in tickers_in:
        with st.spinner(f"{'กำลังโหลด' if TH else 'Loading'} {tk}..."):
            data=get_stock(tk)
        if not data: st.error(f"❌ {'ไม่พบ' if TH else 'Not found'}: {tk}"); continue
        ai=None
        if api_key:
...
if not results:
    ...
    <div style="padding:48px 0 32px;text-align:center">
      ...
      <p style="color:{muted};font-size:1rem;max-width:480px;margin:0 auto 40px;line-height:1.7">
        {'Analyze stocks across 5 dimensions with AI-powered DEEPV Score and BUY/HOLD/AVOID signals.' if not TH else
         'วิเคราะห์หุ้นเชิงลึกด้วย AI ครอบคลุม 5 มิติ พร้อมคะแนน DEEPV Score และ Signal ที่ชัดเจน'}
      </p>
    </div>""", unsafe_allow_html=True)

    cols=st.columns(4)
    feats=[("📐","DEEPV Score",{"TH":"คะแนน 0-100 พร้อมไฟจราจร","EN":"0-100 Score + Traffic Light"}),
           ...
           ("📄","Report",    {"TH":"ดาวน์โหลดรายงาน HTML","EN":"Download HTML Report"})]
    ...
    st.markdown(f"""<div style="background:{bg2};border:1px solid {border};border-radius:12px;padding:24px;margin-bottom:24px">
    ...
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;text-align:center">
      ...
      <div><div style="font-size:1.4rem;font-weight:800;color:#34d399">P</div><div style="font-size:.8rem;font-weight:600;color:{text}">Pricing</div><div style="font-size:.72rem;color:{muted}">{"อำนาจตั้งราคา" if TH else "Pricing Ability"}</div></div>
...
# ══ RESULTS ══════════════════════════════════════════════════════════════════════
    ...
    for i,ticker in enumerate(tr):
        with tabs[i]:
            ...
            <div style="font-size:.78rem;color:{muted};margin-bottom:16px">{d['sector']} · {d['industry']} · {d.get('country','')}</div>""", unsafe_allow_html=True)
            ...
            if d.get("description"):
                with st.expander("📋 " + ("เกี่ยวกับบริษัท" if TH else "About Company"),expanded=False):
                    ...
                    if d.get("employees"): st.metric("Employees",f"{d['employees']:,}")
                    if d.get("website"): st.markdown(f'<a href="{d["website"]}" target="_blank" style="color:#4f7cff">🌐 {d["website"]}</a>', unsafe_allow_html=True)

            st.markdown(f'<div class="slbl">💰 {"ราคา" if TH else "Price"}</div>', unsafe_allow_html=True)
            c1,c2,c3,c4=st.columns(4)
            c1.metric("Price",f"${d['price']:,.2f}"); c2.metric("Prev Close",usd(d.get("prev_close")))
            c3.metric("Day High",usd(d.get("day_high"))); c4.metric("Day Low",usd(d.get("day_low")))
            c5,c6,c7,c8=st.columns(4)
            c5.metric("52W High",usd(d.get("52w_high"))); c6.metric("52W Low",usd(d.get("52w_low")))
            c7.metric("Target Price",usd(d.get("target"))); c8.metric("Market Cap",fmt(d["cap"]))

            st.markdown(f'<div class="slbl" style="margin-top:14px">📐 Valuation</div>', unsafe_allow_html=True)
            v1,v2,v3,v4=st.columns(4)
            v1.metric("P/E (Trail)",xf(d.get("pe"))); v2.metric("P/E (Fwd)",xf(d.get("fwd_pe")))
            v3.metric("PEG",xf(d.get("peg"))); v4.metric("P/B",xf(d.get("pb")))
            v5,v6,v7,v8=st.columns(4)
            v5.metric("P/S",xf(d.get("ps"))); v6.metric("EV/EBITDA",xf(d.get("ev_eb")))
            v7.metric("EPS (Trail)",usd(d.get("eps"))); v8.metric("EPS (Fwd)",usd(d.get("fwd_eps")))

            st.markdown(f'<div class="slbl" style="margin-top:14px">📊 Financials</div>', unsafe_allow_html=True)
            f1,f2,f3,f4=st.columns(4)
            f1.metric("Revenue",fmt(d.get("rev"))); f2.metric("Rev Growth",pct(d.get("rev_g")))
            f3.metric("Gross Margin",pct(d.get("gm"))); f4.metric("Op Margin",pct(d.get("om")))
            f5,f6,f7,f8=st.columns(4)
            f5.metric("Net Margin",pct(d.get("nm"))); f6.metric("ROE",pct(d.get("roe")))
            f7.metric("ROA",pct(d.get("roa"))); f8.metric("Free Cash Flow",fmt(d.get("fcf")))

            st.markdown(f'<div class="slbl" style="margin-top:14px">🏦 Balance Sheet & Risk</div>', unsafe_allow_html=True)
            b1,b2,b3,b4=st.columns(4)
            b1.metric("Total Cash",fmt(d.get("cash"))); b2.metric("Total Debt",fmt(d.get("debt")))
...
<body><h2>{d.get('name',ticker)} ({ticker}) — DEEPV: {overall} | {rec}</h2><p>{ai.get('summary','')}</p></body></html>"""
    ...
    # ── Portfolio ─────────────────────────────────────────────────────────────────
    with tabs[-1]:
        st.markdown(f"### 💼 {'Portfolio' if not TH else 'Portfolio Simulator'}")
            ...
            if st.button("➕ " + ("เพิ่มหุ้น" if TH else "Add")):
                if pt_:
                    if pt_ in [p["ticker"] for p in st.session_state.portfolio]:
                        ...
                        if pd_:
                            st.session_state.portfolio.append({"ticker":pt_,"name":pd_["name"],"alloc":pal_,
                                "price":pd_["price"],"change":pd_["change"],"sector":pd_["sector"],
                                "pe":pd_.get("pe"),"beta":pd_.get("beta")})
                            st.rerun()
                        else: st.error(f"❌ {pt_}")
            ...
            for x,r in results.items():
                if x not in ex and r["data"]:
                    ...
                    st.session_state.portfolio.append({"ticker":x,"name":d_["name"],"alloc":ea,
                        "price":d_["price"],"change":d_["change"],"sector":d_["sector"],
                        "pe":d_.get("pe"),"beta":d_.get("beta")})
            st.rerun()
        ...
        if not port:
            ...
            st.markdown(f'<div style="text-align:right;font-size:.85rem;color:{ac};margin-bottom:12px">{"รวม" if TH else "Total"}: <strong>{total}%</strong> {"✅" if total==100 else "⚠️"}</div>',unsafe_allow_html=True)
            st.markdown("---")
